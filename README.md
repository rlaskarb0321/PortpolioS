...
로드할 SO 는 주소를 직접 들고 다니지 않고 **캐릭터 이름을 주소 서식에 끼워** 특정합니다. 인스펙터에는 `Data/Config/Combat/{0}` · `Data/Config/Stat/{0}` 두 서식만 두고, `PlayableCharacterInfoSO` 에서 얻은 이름을 넣습니다. 여러 명이 같은 캐릭터를 골라도 이름으로 걸러 한 번만 로드합니다.

로드는 로딩 화면이 떠 있는 `OnSceneActivated` 단계에서 `UniTask` 로 진행되어 메인 스레드를 막지 않습니다.
